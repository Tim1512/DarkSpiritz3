...
os.system('clear')
ex = os.path.exists('./.git')
if ex == True:
    print(Command_exe("["+time.strftime('%H:%M:%S')+"] Checking for Version Update                    ",'git pull'))
    os.system('git pull')
elif ex == False:
    print("Its reccomended to pull from the repo on github in order to stay updated and receive hotfixes..")
    pass
print("")
...
if CHECK_LOGS == True:
    if os.path.exists("./storage/logs/config.xml") == False:
        ...
        os.system('sudo cp /storage/config-skeleton.xml /storage/logs/config.xml')
        retarget.edit()
        ...
        if terminal[0:3] =='use':
            if terminal[4:] == terminal[4:]:
                ...
                if os.path.exists(Exploit_check) == True:
                  os.system("python3 %s.plugin" % (terminal[4:]))
                  main()
                else:
